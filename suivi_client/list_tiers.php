 ...
 *								- Nom générique si oublie saisie nom du 
...
if ('GETPOST' == 'GETPOST') {
	...
	// 
		...
		// arrivée par le Tiers et Suivi des Echanges
		$tiers = trim(GETPOST("search_tiers", 'int'));
		if (!empty($tiers)) {
			$Reftiers = $tiers;
			$socid = $Reftiers;
		}
		if (empty($socid)) $socid = $Reftiers;
		$arg_idtiers=$Reftiers;
		$arg_nomtiers = trim(GETPOST("arg_nomtiersCart", 'alpha'));

		if ($arg_idtiers == -1) $arg_idtiers = '';
		$arg_nominterl=trim(GETPOST("arg_nomtiersCart", 'alpha'));
		$arg_idInterlo = GETPOST('arg_idtiersLig');	
...
print '<p>DEBUG INit arg_mailInterl:'.$arg_mailInterl.'</p>';	
	...
	*/
	...
	$arg_description=trim(GETPOST("arg_description", 'alpha'));
	...
	if (empty($sortfield))       {
		if ($TypeListe == 'generale')  $sortfield=" spri.ordre , dateAff";
		else $sortfield=" e.datec";
	}
	if (empty($sortorder) or $sortorder == "") $sortorder=" desc ";
...
if ('Variable'=='Variable') {
	...
	$locationUrlRetour= "Location: " . $_SERVER ['PHP_SELF'] . "?typeliste=tiers".$param ;
...
if ($btaction =='Enregistrer'  or $btaction =='EnrRetCah'  ){ 
	...
	if ($arg_id_dossier == -1  and empty($arg_dossier) and !$fglDossiervide ) {
		$error++;	
		$texterror = 'errors'; 
		setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("dossier")),'',$texterror);
	}
	// TIERS 
	if (!( $arg_dossanstiers == 'oui' and ($arg_id_dossier <> -1 or empty($arg_dossier))) ) // ne rien faire si on a un nouveau dossier sans tiers dans l'écran general
	{
		if (!empty($arg_nvtiers) or !empty($arg_idtiers)) {
			...
			elseif ($ret > 0) {
			/*	if ($TypeListe <> 'tiers') $arg_idtiers = $ret;
				if ($TypeListe == 'dossier') { // il se peut que l'on vienne de changer le tiers du dossier
	...
	if (( $arg_idInterlo <> -1 AND !empty($arg_idInterlo) and empty($arg_idtiers) ) 
		or ( $arg_idInterlo <> -1 AND!empty($arg_idInterlo) and !empty($arg_idtiers) and $arg_idtiers <> $arg_idInterlo)
	...
	if ($error == 0 and empty($texterror) and !$fglDossiervide) {
		...
		//if (!($arg_id_dossier <> -1 and !empty($arg_nvdossier) and $TypeListe <> 'dossier')) {
			...
			if ($arg_id_dossier <> -1 and !empty($arg_dossier) )  print ' Cas1 DOSSIER non traité';
			if ($ret < 0) {
					$error++;
...
if ('OrdreSql'=='OrdreSql') {
	...
	// pour Tiers, il faut aller chercher les info dans un SQL spécifique dépendant de arg_idtiers
		...
		$sql = "SELECT DISTINCT   st.rowid, st.nom as nomTiers, st.phone as TiersTel, st.email, stex.s_tel2 as TiersSupTel,  stex.s_email2 as TiersSupMail, p.code as country_code ";
...
//		$sql .= " WHERE st.rowid = ' ".$arg_idtiers."'";
		$sql .= " WHERE st.rowid = ' ".$Reftiers."'";
		$resql = $db->query($sql);
		$objsoc = $db->fetch_object($resql); 
		$linesoc->fk_tiers			= $objsoc->rowid;
		$linesoc->nomTiers		 	= $objsoc->nomTiers 	;
		$linesoc->TiersTel			= $objsoc->TiersTel;
		$linesoc->TiersSupTel		= $objsoc->TiersSupTel;
		$linesoc->TiersMail			= $objsoc->email;
		$linesoc->TiersSupMail		= $objsoc->TiersSupMail;		
		$linesoc->country_code		= $objsoc->country_code;
	...
	$sql .= " AND  ( d.fk_soc = '".$Reftiers."' or e.fk_soc = '".$Reftiers ."')";
...
if ('Charge' == 'Charge') {
		$obj = $db->fetch_object($resql);
			...
			$line_echange->date	= max( $obj->EchDate, $obj->Echtms);
...
* ENTETE
****************************************************/ 
 llxHeader('',$langs->trans('TiSuiviEchanges'));
...
if ('Entete' == 'Entete') {
	...
	// TITRE
	if (empty($listtotal)) $titre = 'CS_ListeTiersTitrePar';
	else $titre = 'CS_ListeTiersTitreTot';
...
if ('Liste' == 'Liste') {
	...
	if (empty($btaction ) or $btaction == 'create'  or $btaction == 'Enregistrer' or $btaction == 'Supprime' or $btaction == 'edit'  or $btaction == 'ConfSupprime') {
		...
		if (! empty($btaction ) and (($btaction == 'Enregistrer' and $error == 0 )  or $btaction == 'ConfSupprime'))
			$waff->AfficheLignedossier($TypeListe,'','' ,1, 'saisie', $listtotal);	// pour création nouv dossier/echange
		...
		//ligne ci dessus : affichage d'une saisie nouv dossier/echange qui a queuter et qu'on réaffiche pour merttre l'enregistrement			
	}	
