...
if (! empty($action) && $action == 'fetch' && ! empty($id))
{
		...
		if (! empty($price_by_qty_rowid) && $price_by_qty_rowid >= 1 && (! empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY))) 		// If we need a particular price related to qty
		{
			...
			if ($result) {
				...
				if ($objp) {
					...
					$outprice_ttc = price($objp->unitprice * (1 + ((int)$object->tva_tx / 100)));
	...
	if (! GETPOST($htmlname,'alpha') && ! GETPOST($idprod,'int'))
		return;

		// When used from jQuery, the search term is added as GET param "term".
	$searchkey = (GETPOST($idprod,'int') ? GETPOST($idprod,'int') : (GETPOST($htmlname,'alpha') ? GETPOST($htmlname,'alpha') : ''));
